...
def train(x, cond, modules, optimizer, kl_anneal, args):
    ...
    for i in range(args.n_past + args.n_future):
        # x.shape = (batch_size, 12, 3, 64, 64) 
        x_i = x[:, i, :, :, :]
        # Encode the whole x_i sequence into h_i 
        h_i.append(modules['encoder'](x_i))
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        # h_t-1 and skip
        if (args.last_frame_skip== True) or (i < args.n_past):	
            h_prev, skip = h_i[i-1]
        else:
            h_prev = h_i[i-1][0]
        
        # Put h_t into gaussian LSTM and get z_t, mean and variance
        z_t, mean, variance = modules['posterior'](h_t)
        
        # Combine conditions, h_t-1 and z_t together and feed in LSTM
        lstm_input = torch.cat([cond[:, i-1, :], h_prev, z_t], 1)
        g_t = modules['frame_predictor'](lstm_input)
        
        # Put g_t into the decoder and get x_t_hat
        x_t = modules['decoder']([g_t, skip])
        
        # If teacher forcing isn't used, replace the next input into x_t_hat
        if use_teacher_forcing==False :
            h_i[i] = modules['encoder'](x_t)
        
        # Calculate MSE loss and KL divergence
        mse += mse_loss(x_t, x[:, i, :, :, :])
        kld += kl_criterion(mean, variance, args)
...
def pred(x, cond, modules, args, device):
    ...
    with torch.no_grad():
        ...
        # Encode the whole x_i sequence into h_i 
        for i in range(args.n_past+args.n_future):
            h_i.append(modules['encoder'](x[i]))
        
        # h_t-1 and skip
        for i in range(1, args.n_past+args.n_future):
            
            if (args.last_frame_skip==True):
                h_prev, skip = h_i[i-1]
            ...
            if i < args.n_past:
                # Put h_t into gaussian LSTM and get z_t, mean and variance
                z_t, mean, variance = modules['posterior'](h_i[i][0])
                lstm_input = torch.cat([cond[i-1], h_prev, z_t], 1)
                modules['frame_predictor'](lstm_input) 
                ...
                # Put g_t into the decoder and get x_t_hat
                x_t = modules['decoder']([g_t, skip])
                # Always use the previous prediction as last frame
                h_i[i] = modules['encoder'](x_t)
                prediction.append(x_t)
...
class kl_annealing():
    def __init__(self, args):
        ...
        self.L = np.ones(self.niter)
        self.i = 0

        if self.kl_anneal_cyclical:
            period = self.niter / args.kl_anneal_cycle
            step = 1 / (period * args.kl_anneal_ratio)
        else:
            period = self.niter
            step = 1 / (period * args.kl_anneal_ratio)

        for cycle in range(1 if not self.kl_anneal_cyclical else args.kl_anneal_cycle):
            i, j = 0, 0
            while i <= 1 and int(j + cycle*period) < self.niter:
                self.L[int(j + cycle*period)] = i
                i += step
                j += 1
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for i, (x, condition) in enumerate(tqdm(train_loader)):
            
            x = x.to(device)
            condition = condition.to(device)
            loss, MSE, KLD = train(x, condition, modules, optimizer, kl_anneal, args)
            epoch_loss += loss
            epoch_mse += MSE
            epoch_kld += KLD
        ...
        if epoch >= args.tfr_start_decay_epoch:
            ...
            teacher_forcing_ratio = (1.0 - (epoch - args.tfr_start_decay_epoch) * 
                                     (1.0 - args.tfr_lower_bound) / (args.niter - args.tfr_start_decay_epoch) *
                                     args.tfr_decay_step)
            
            if teacher_forcing_ratio < args.tfr_lower_bound:
                args.tfr = args.tfr_lower_bound
                
            elif teacher_forcing_ratio > 1:
                args.tfr = 1
            else:
                args.tfr = teacher_forcing_ratio
            
        print("Teacher Forcing Ratio: ")
        print(args.tfr)
